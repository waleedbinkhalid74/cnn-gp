...
#%%
# grad_1 = frechet(mu, X_random, Y, sample)
# grad_2 = frechet_rbf(mu, X_random, Y, sample, batch)
#%% 
...
class KernelFlowsNP():
    ...
    def fit(self, X, Y, iterations, batch_size = False,learning_rate = 0.1, type_epsilon = "relative", show_it = 100, record_hist = True, reg = default_lambda):
        ...
        for i in tqdm(range(iterations)):
            ...
            g, rho = frechet(parameters, X_batch, Y_batch, sample_indices, kernel_keyword = self.kernel_keyword)
            if rho >1 or rho <0:

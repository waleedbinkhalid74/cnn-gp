...
def frechet_grad(parameters, X_data, Y_data, sample_indices, kernel_keyword= "RBF", regu_lambda = 0.000001):
    ...
    y_hat = np.matmul(inverse_data, Y_data)
    z_hat = np.matmul(np.transpose(pi), np.matmul(inverse_sample, np.matmul(pi, Y_data)))
    ...
    #     print(2)
    #     gradient = np.trace(gradient)
    # elif len(gradient.shape) == 3:
    #     print(3)
    #     gradient = np.trace(gradient, axis1= 1, axis2=2)
    # elif len(gradient.shape) == 4:
    #     gradient = np.trace(gradient, axis1 = 2, axis2=3)
    gradient = -gradient/np.trace(bottom)

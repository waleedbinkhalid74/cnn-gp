...
class KernelFlowsTorch():
    ...
    def batch_creation(dataset_size:int, batch_size: int, sample_proportion: float = 0.5) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if batch_size == False:
            ...
            batch_indices = KernelFlowsTorch.sample_selection(dataset_size, batch_size)
        ...
        sample_indices = KernelFlowsTorch.sample_selection(data_batch, sample_size)
    ...
    def kernel_regression(X_test: torch.Tensor, X_train: torch.Tensor,
        ...
        with torch.no_grad():
            k_matrix = KernelFlowsTorch._block_kernel_eval(X=X_train,
                                                            Y=X_train,
                                                            blocksize=blocksize,
                                                            kernel=kernel,
                                                            device=device,
            ...
            t_matrix = KernelFlowsTorch._block_kernel_eval(X=X_test,
                                                            Y=X_train,
                                                            blocksize=blocksize,
                                                            kernel=kernel,
                                                            device=device,
    ...
    def _rho_bo(self, batch_size, sample_proportion, params) -> torch.Tensor:
        ...
        with torch.no_grad():
            theta = KernelFlowsTorch._block_kernel_eval(X=X_batch,Y=X_batch,kernel=self.cnn_gp_kernel,
    ...
    def predict(self, X_test: torch.Tensor, N_I: int = False) -> torch.Tensor:
        ...
        if N_I is False:
            _, batch_indices = KernelFlowsTorch.batch_creation(dataset_size= self._X.shape[0],
        ...
        prediction = KernelFlowsTorch.kernel_regression(X_train= X_batch,
                                                        Y_train= Y_batch,
                                                        X_test=X_test,

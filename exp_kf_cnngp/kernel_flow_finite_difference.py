...
def main(_):
    ...
    if FLAGS.dataset == 'mnist' and FLAGS.CNNGP_model == 'convnet':
        convnet_alonso_acc = []
        convnet_alonso = kernel_flow_configs.get_CNNGP('alonso_etal_convnet')
        for N_i in tqdm(N_i_arr):
            Y_predictions_trained = KernelFlowsTorch.kernel_regression(X_test=X_test, X_train=X_train[:N_i], 
                                                                                    Y_train=Y_train[:N_i], kernel=cnn_gp, 
                                                                                    regularization_lambda=0.0001, blocksize=250, 
                                                                                    device=DEVICE)
            Y_predictions_trained_labels = np.argmax(Y_predictions_trained.cpu(), axis=1)
            convnet_alonso_acc.append(accuracy_score(Y_predictions_trained_labels, Y_test.cpu().numpy()) * 100)
        ax.plot(N_i_arr, convnet_alonso_acc, '-o', label='ConvNet-GP with parameters from Garriga-Alonso et al')
